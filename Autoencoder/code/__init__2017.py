...
def readdata():
    ...
    cursor = db.cursor()
    try:
        cursor.execute("SELECT MMSI,BaseDateTime FROM ais_2017 WHERE ID= '{0}';".format(ID))
        AAA=cursor.fetchall()
        MMSI = AAA[0][0]
        ...
        print("读取起点错误")
    # SQL 查询语句
    sql_truth = "SELECT * FROM ais_2017 WHERE  MMSI = '{0}' order by BaseDateTime ;".format(MMSI)
    sql_truth_point = "SELECT * FROM ais_2017 WHERE  MMSI = '{0}'  and BaseDateTime = '{1}';".format(MMSI,Basedatetime)
    try:
        # 执行SQL语句,获得预测真实航迹
...
def zuobiaozhou(first_point2=[]):
    ...
    sql_result = "SELECT * FROM ais_2017 WHERE lat BETWEEN {0} AND {1} AND lon BETWEEN {2} AND {3} AND MMSI!='{4}';".format(
...
def getTrajectory(S0, first_point):
    ...
    for i in range(len(S0)):
        ...
        cursor = db.cursor()
        # SQL 查询语句
        sql_S0 = "SELECT * FROM ais_2017 WHERE  MMSI = '{0}' and  lat BETWEEN {1} AND {2} AND lon BETWEEN {3} AND {4} and BaseDateTime between '{5}' and '{6}' order by BaseDateTime;".format(
...
def LDA_4(back_trajectory_with_label, back_select):
    ...
    LDA_back = [[MMSI[i]] + list(back_trajectory_chain[i]) + [label[i]] for i in range(len(MMSI))]
    LDA_select = [[back_select[0][0]] + list(back_trajectory_chain[0])]
...
# 轨迹预测画图
def draw_prediction(real_trajectory,pre_trajectory,back_select,first_point):
    ...
    for point in back_select:
        x, y = map(float(point[3]), float(point[2]))
        map.plot(x, y, marker='.', color='black', markersize=1)  # 黑色代表back_select
    print("真实画图成功")
...
def out():
    ...
    PCA_back = PCA_4(back_trajectory)
    # cluster_trajectory = clustering_trajectory(PCA_forward)
    cluster_trajectory = clustering_trajectory(PCA_back)
    ...
    draw_prediction(forward_select,pre_trajectory,back_select,first_point)
