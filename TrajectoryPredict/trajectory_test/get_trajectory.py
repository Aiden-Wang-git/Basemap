...
trajectories_two = np.load('trajectories_process2.npy', allow_pickle=True).item()
# drawTrajectory("第一章提取到的航迹数据", trajectories_two)


trajectories_one = np.load('trajectories_one_month.npy', allow_pickle=True).item()
# labels = np.load('labels.npy',allow_pickle=True)
# i = 0
# for key in trajectories_one:
#     trajectories_one[key].label = labels[i]
#     i = i+1
# np.save('trajectories_one_month.npy',trajectories_one)
# drawTrajectory("一月份航迹数据", trajectories_one)
# ans = []
# count = 0
# keys = random.sample(trajectories_two.keys(), 20)
# np.save('keys_two',keys)
# for key2 in keys:
#     print(count)
#     count = count+1
#     points1 = trajectories_two[key2].points[:40]
#     traDistance = []
#     for key1 in trajectories_one:
#         points2 = trajectories_one[key1].points[:40]
#         traDistance.append([DTW(points1, points2),trajectories_one[key1].label])
#     traDistance = sorted(traDistance,key=(lambda x:x[0]),reverse=False)
#     ans.append(traDistance[:20])
# np.save('min_distance',ans)
ans = np.load('min_distance.npy',allow_pickle=True)
keys = np.load('keys_two.npy',allow_pickle=True)
print(ans)
# 用于测试的航迹key
keys_test = ['367688000-0','477014700-0','538004318-0','477271300','477444500','538004007','211327410-0','309484000-1','367393270-10']
# 用于测试的航迹的label
label_test = [7,3,7,6,6,7,3,5,2]
trajectories_test = {}
for key in keys_test:
    trajectories_test[key] = trajectories_two[key]
    trajectory = trajectories_two[key]
    with open(f"{key}.csv", 'a+', newline='') as csvfile:
...
drawTrajectory("需要预测的航迹分布", trajectories_test)